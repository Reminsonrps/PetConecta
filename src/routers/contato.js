...
router.post("/contatos", async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Contato adicionado com sucesso", id: docRef.id });
  } catch (error) {
...
router.get("/contatos", async (req, res) => {
  try {
    const snapshot = await contatosCollection.limit(100).get();
    ...
    res
      .status(500)
      .json({ message: "Erro ao listar contatos", error: error.message });
  }
});
